...
export default function TimesOfIndiaTracker({ isAdmin = false }) {
  ...
  const defaultFlats = [
    {
      ...
      flatNo: 'A-302',
      bills: [
        { id: 1, period: 'Oct 2021 - Jun 2025', amount: 2200, status: 'Pending' },
        { id: 2, period: 'Jul 2025 - Jun 2026', amount: 3000, status: 'Pending' }
      ]
    },
    {
      id: 'A-904',
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        console.error('TOI load error:', err);
        setFlats(defaultFlats);
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const saveToFirebase = async (data) => {
    ...
    try {
      await ensureFirebaseSession();
      await setDoc(doc(db, 'toiTracking', recordId), {
        flats: data,
  ...
  return (
    <div className="tab-pane active" style={{ maxWidth: '900px', margin: '0 auto', padding: '20px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 style={{ margin: '0 0 8px 0', fontSize: '1.5rem', fontWeight: '600' }}>Times of India Bills</h2>
          <p style={{ margin: 0, color: '#6b7280' }}>Track newspaper subscription bills for specific flats</p>
        </div>
              ...
              <div style={{ 
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={{ 
                    ...
                    {totalPending > 0 ? `Pending: ${fmt(totalPending)}` : 'All Paid'}
                  </span>
                </div>
              </div>
