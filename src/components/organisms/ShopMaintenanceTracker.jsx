...
export default function ShopMaintenanceTracker() {
  ...
  const handleInsertSubmit = (e) => {
    e.preventDefault();
    if (!insertMonth || selectedShops.length === 0) return;
    
    const regAmt = insertRegularAmount ? Number(insertRegularAmount) : undefined;
    const recAmt = insertAmount ? Number(insertAmount) : undefined;
    ...
    setShops(prevShops => {
      return prevShops.map(shop => {
        ...
        if (monthIndex >= 0) {
          if (regAmt !== undefined) newLedger[monthIndex].regularMain = regAmt;
          if (recAmt !== undefined) newLedger[monthIndex].receipts = recAmt;
        } else {
          newLedger.push({ 
            month: insertMonth, 
        ...
        for (let i = 0; i < newLedger.length; i++) {
          runningBalance += (newLedger[i].regularMain || 0) - (Number(newLedger[i].receipts) || 0);
          newLedger[i].netAmount = runningBalance;
        }
        
        return { ...shop, ledger: newLedger };
      });
    });
      ...
      <div style={{ background: '#fff', borderRadius: 18, padding: 20, border: '1px solid rgba(61,63,52,0.08)', boxShadow: '0 8px 24px rgba(16,24,40,0.06)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 16 }}>
          {[
            ...
            { label: '🧾 Monthly Maintence Total', value: formatValue(totalRegularMain), sub: 'Total Due', accent: '#196c6c' },
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr>
                          ...
                          <th style={{ padding: '11px 16px', borderBottom: '2px solid rgba(61,63,52,0.1)', color: '#5f665f', fontWeight: 700, fontSize: '0.66rem', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap', background: 'rgba(244,239,231,0.7)', textAlign: 'left' }}>Monthly Maintence</th>
      ...
      {isInsertModalOpen && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10vh' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: '100%', maxWidth: 400, boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: '1.2rem', color: '#101828' }}>Add Shop Maintance</h3>
            <form onSubmit={handleInsertSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#344054', marginBottom: 4 }}>Monthly Maintence Amount</label>
                <input 
                  type="number" 
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                ...
                <button 
                  type="submit"
                  style={{ flex: 1, padding: '10px', borderRadius: 8, background: '#0b2b26', border: 'none', color: '#fff', fontWeight: 600, cursor: 'pointer' }}
                >
                  Save Receipts
                </button>
              </div>
